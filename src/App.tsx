...
function isStillUnlocked() {
  const unlocked =
    sessionStorage.getItem("capital-lumber-unlocked") === "true";
...
export default function App() {
  ...
  const [lastListPage, setLastListPage] =
    useState<Page>("decking");
  const [selectedRow, setSelectedRow] =
    useState<PriceRow | null>(null);
  ...
  const vendors = useMemo(() => {
    const set = new Set(
      data.rows.map((r) => r.vendor).filter(Boolean)
    );
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.rows.filter((r) => {
      ...
      const matchesFire = fireOnly
        ? flagTrue((r as any).fireRated)
        : true;

      const matchesCool = coolOnly
        ? flagTrue((r as any).coolRated)
        : true;
  ...
  function openDetail(row: PriceRow) {
    setSelectedRow(row);
    if (page === "decking" || page === "fascia") {
      setLastListPage(page);
    }
    setPage("detail");
  }
