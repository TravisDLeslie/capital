...
export default function App() {
  ...
  const [coolOnly, setCoolOnly] = useState(false); // ✅ NEW
  ...
  function go(next: Page) {
    ...
    setQuery("");
    setFireOnly(false);
    setCoolOnly(false); // ✅ reset on nav
  }
    ...
    useMemo(() => {
      if (page === "fascia") {
        return {
          ...
          description:
            "View fascia pricing by size, series, brand, vendor, and color. Cost is your cost/ft. Price is calculated by tier.",
      ...
      return {
        ...
        description:
          "View decking pricing by size, series, brand, vendor, and color. Cost is your cost/ft. Price is calculated by tier.",
  ...
  {page === "calculator" ? (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-2 max-w-4xl text-sm text-slate-600">
          Build a quote from multiple areas (sqft or dimensions), add waste, tax,
          delivery, and export PDF.
        </p>
      </div>

      {/* ✅ Calculator uses decking rows */}
    ...
    <div className="space-y-4">
      ...
      <p className="mt-2 text-sm text-slate-600">
        Use the Settings tab to view import commands and store internal notes.
      </p>
      {/* If you have Settings component, render it here */}
      {/* <Settings settingsPin={SETTINGS_PIN} /> */}
    </div>
  ) : (
    <>
      {/* Decking / Fascia Title */}
      <div>
...
<div className="flex items-center">
  <button
    ...
    title="Show cool-rated products only"
  >
    ...
    {coolOnly && (
      ...
      <section className="mt-6 w-full rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="px-5 py-4">
          <PriceTable rows={filteredRows} tier={tier} sortDir={sortDir} />
        </div>
      </section>
    </>
  )}
