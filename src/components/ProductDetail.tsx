...
export default function ProductDetail({
  ...
  const fire = flagTrue((row as any).fireRated ?? (row as any).fire ?? (row as any).fire_rating);
  const cool = flagTrue((row as any).coolRated ?? (row as any).cool ?? (row as any).cool_rating);
      ...
      <div className="mt-5 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="aspect-[4/3] w-full overflow-hidden rounded-xl bg-slate-50">
            {row.image ? (
              <img src={row.image} alt={`${row.color ?? "Product"} large`} className="h-full w-full object-cover" />
            ) : (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-2 text-sm font-semibold text-slate-700">{row.color}</div>
